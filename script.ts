...
async function programa() {
  const analisador = new Analisador("Perfil.xlsx", 'out');

  analisador.tratar_dados();
  analisador.exibir_análise();
  await analisador.renderizar_gráficos();
...
class Analisador {
    ...
    for (const informação of this.lista_de_informações) {
      for (const pergunta in informação) {
        ...
        if (estáDuplicada) {
          ...
          this.registrar_informacao(pergunta, resposta, elementos)
        }
  ...
  public async renderizar_gráficos() {
    ...
    for (const pergunta in this.interações) {
      ...
      }, pergunta, respostas, contagem, contador_de_perguntas) as {imagem_em_base64: string, configuração_em_json: string};

      if (resultado) {
        const { imagem_em_base64, configuração_em_json } = resultado;

        const dados = String(imagem_em_base64).split(',')[1];
        ...
        configurações_em_json.push(configuração_em_json);
      }
    }

    await página.evaluate((configurações_em_json) => {
      configurações_em_json.forEach((configuração_em_json, índice) => {
        const número_do_canvas = índice + 1;

        document.body.innerHTML += `
          <script>
            const canvas${número_do_canvas} = document.getElementById('${número_do_canvas}');
            const contexto${número_do_canvas} = canvas${número_do_canvas}.getContext('2d');
            const configuração${número_do_canvas} = ${configuração_em_json};
            
            new Chart(contexto${número_do_canvas}, configuração${número_do_canvas});
          </script>
        `;
      });
    }, configurações_em_json);

    const código = await página.evaluate(() => {
    ...
    if (código) {
      ...
      console.log('\x1b[32mSalvando\x1b[0m', nome_do_arquivo);
      fs.writeFileSync(nome_do_arquivo, código);
    }
  ...
  private registrar_informacao(pergunta: string, resposta: string, elementos: Array<string> | undefined) {
  ...
  private static CATEGORIAS: {[chave: string]: string} = {
  ...
  private static PALAVRAS_CHAVE: {[chave: string]: string} = {
